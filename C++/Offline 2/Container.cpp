...
class Container{
    ...
    void reset(){
        ...
        if (valueMatrix != NULL){
            ...
            {
                if(valueMatrix[i]!=NULL)delete[] valueMatrix[i];
                valueMatrix[i]=NULL;
            }
            delete[] valueMatrix;
        }
...
public:
    ...
    Container (int **valMat, int r, int c){
        ...
        for(int i=0;i<r;i++)
        {
            ...
            for(int j=0;j<c;j++)valueMatrix[i][j]=valMat[i][j];
        }
        firstDim = r;
        secondDim = c;
    ...
    Container(const Container &ob)
    {
        ...
        secondDim = 0;
        storedType = -1;
        if(ob.storedType==INTEGER)setItem(*ob.value);
        if(ob.storedType==INT_ARRAY)setItem(ob.valueArray,ob.firstDim);
        if(ob.storedType==INT_MATRIX)setItem(ob.valueMatrix,ob.firstDim,ob.secondDim);
    }
